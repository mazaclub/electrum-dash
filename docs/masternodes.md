...
Electrum-ION supports masternode creation through an interface called the Masternode Manager.
...
- The ION address that received the collateral payment.
- The ION address of the delegate key.
...
A masternode requires a "delegate" key, which is known to both Electrum-ION and your masternode.
Your masternode will use this key to sign messages, and the ION network will know that you authorized
it to.

A delegate key can either be one of your Electrum-ION keys, or an imported key. Either way, your masternode
and Electrum-ION will both need to know the private key. (See *Importing Masternode.conf* below.)

To use one of your Electrum-ION keys as a delegate key, put its address in the `Masternode ION Address`
...
### Collateral (1000 ION Payment)

To start a masternode, you must have a 1000 ION payment available in your wallet.
You can scan your wallet for 1000 ION payments in the `Activate Masternode` tab of the Masternode
Manager.

After scanning, a list of available 1000 ION collateral payments will be displayed. Selecting one
...
waiting for Electrum-ION to sign and broadcast your masternode announcement, you will be presented with
